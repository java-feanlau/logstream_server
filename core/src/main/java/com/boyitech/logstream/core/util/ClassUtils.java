...
public class ClassUtils {
	...
	/**
	 * 获取某包下所有类
	 * @param rootPath 根目录路径名
	 * @param packageName 包名
	 * @param isRecursion 是否遍历子包
	 * @return 类的完整名称
	 */
	public static Set<String> getClassName(String rootPath, String packageName, boolean isRecursion) {
		String path = rootPath + "/" + packageName.replace(".", "/");
		Set<String> classNames = getClassNameFromDir(path, packageName, isRecursion);
		return classNames;
	}
	...
	public static Set<String> getClassNameFromDir(String filePath, String packageName, boolean isRecursion) {
		...
		for (File childFile : files) {
			if (childFile.isDirectory()) {
				...
				} else if(fileName.endsWith(".java") && !fileName.contains("$") ) {
