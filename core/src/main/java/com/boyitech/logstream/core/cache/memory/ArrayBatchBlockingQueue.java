...
import com.boyitech.logstream.core.cache.queue.BatchBlockingQueue;
...
public class ArrayBatchBlockingQueue<E> extends AbstractQueue<E> implements BatchBlockingQueue<E> {

	private static final long serialVersionUID = -1205062024016069355L;
    ...
    /**
     * Creates an {@code ArrayBlockingBatchQueue} with the given (fixed)
    ...
    /**
     * Iterator for ArrayBlockingBatchQueue.
     *
    ...
    private class Itr implements Iterator<E> {
        ...
        Itr() {
            ...
            lastRet = NONE;
            final ReentrantLock lock = ArrayBatchBlockingQueue.this.lock;
            lock.lock();
            try {
                if (count == 0) {
                    // assert itrs == null;
                    ...
                    final int takeIndex = ArrayBatchBlockingQueue.this.takeIndex;
...
//         }
     ...
     * Call only when holding lock.
     */
    private void enqueueNoNotify(E x) {
        // assert lock.getHoldCount() == 1;
        // assert items[putIndex] == null;
        final Object[] items = this.items;
        items[putIndex] = x;
	...
	public List<E> poll(int batch) {
		...
		try {
			int i = 0;
			while ((!this.isEmpty() & i < batch)) {
				list.add(this.dequeueNoNotify());
				i++;
			}
			return list;
		} finally {
